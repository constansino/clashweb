...
`clashweb` 是一个基于 [Zephyruso/zashboard](https://github.com/Zephyruso/zashboard) 3.21 的 Mihomo 控制面板分支。项目保留上游的代理组文件夹、全局/组内搜索、连接表格、测速、设置与多后端架构，只补充上游暂未覆盖的设备级策略和规则热覆写。
...
| ---------------------------------------- | ------------------- | ------------------------------------------- |
| 策略组文件夹、搜索模式、连接表格、多后端 | 支持                | 直接采用上游实现                            |
